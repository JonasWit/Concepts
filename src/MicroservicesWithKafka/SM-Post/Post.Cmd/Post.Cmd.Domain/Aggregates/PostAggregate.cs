...
public class PostAggregate : AggregateRoot
{
    ...
    {
        if (!Active) throw new InvalidOperationException("You cannot edit the message of an inactive post");
    ...
    {
        if (!Active) throw new InvalidOperationException("You cannot like an inactive post");
    ...
    {
        _id = @event.Id;
        _comments[@event.CommentId] = new ValueTuple<string, string>(@event.Comment ?? "", @event.Username ?? "");
    }
    ...
    {
        if (!Active) throw new InvalidOperationException("Post was removed");
